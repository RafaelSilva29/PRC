<template>
  <div>
    <v-card class="ma-2">
      <v-card-title class="indigo darken-4 white--text" dark>
        Cinemateca de PRC2020: Lista dos Filmes na BD
        <v-spacer></v-spacer>
        <v-text-field
          v-model="filtrar"
          label="Filtrar"
          single-line
          hide-details
          dark
        ></v-text-field>
      </v-card-title>
      <v-card-text>
        <v-data-table
          :headers="hfilmes"
          :items="filmes"
          :footer-props="footer_props"
          :search="filtrar"
        >
          <template v-slot:no-data>
            <v-alert :value="true" color="#FFA500" icon="warning">
              Ainda não foi possivel carregar os dados...
            </v-alert>
          </template>
          <template v-slot:item.ops="{ item }">
            <v-icon
              @click="mostraFilme(item)"
            >
              {{ verFilme }}
            </v-icon>
          </template>
        </v-data-table>
      </v-card-text>
    </v-card>
    <div class="last"/>
  </div>
</template>


<script>
import { mdiMovieOpen } from '@mdi/js';

export default {
  name: 'ListaFilmes',
  data: () => ({
    hfilmes: [
      { text: 'Título', sortable: true, value: 'titulo', align: 'center', class: 'subtitle-1' },
      { text: 'Data', sortable: true, value: 'data', align: 'center', class: 'subtitle-1' },
      { text: 'Duração', sortable: true, value: 'duracao', align: 'center', class: 'subtitle-1' },
      { text: 'Popularidade', sortable: true, value: 'pop', align: 'center', class: 'subtitle-1', filterable: false },
      { text: 'Operações', align: 'center', value: 'ops' },
    ],
    footer_props: {
      "items-per-page-text": "Mostrar",
      "items-per-page-options": [10, 20, 50, 100, -1],
      "items-per-page-all-text": "Todos"
    },
    filtrar: "",
    filmes: [],
    verFilme: mdiMovieOpen
  }),
  created: async function() {
    try {
      const response = await this.axios.get('/filmes');
      this.filmes = response.data
    } catch (error) {
      console.log(error)
    }
  },
  methods: {
    mostraFilme(filme){
      const idFilme = filme.idFilme;
      this.$router.push({ name: 'FilmeIndividual', params: { idFilme: idFilme }});
    },
  },
}
</script>

<style lang="scss">
.last{
  height: 80px;
}
</style>
